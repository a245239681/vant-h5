  ...
  methods: {
    ...
    logout() {
      ...
      that.$dialog
        .confirm({
          title: '',
          message: '是否确认注销社保卡号为' + that.info.card + '的社保卡',
        })
        .then(() => {
          that.$http.postJson('/api/gxrswx/Card/cardCloseAccount', {}, res => {
            if (res.data.code === 0) {
              that.$toast.success('注销成功')
              setTimeout(() => {
                that.getUserinfo()
              }, 2000)
              this.info.text = '已注销'
              ...
              that.$dialog.alert({ message: '注销失败，稍后重试' })
            }
          })
        })
        .catch(() => {})
